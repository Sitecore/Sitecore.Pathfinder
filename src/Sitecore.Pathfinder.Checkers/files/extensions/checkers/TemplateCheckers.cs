    ...
    public class TemplateCheckers : Checker
    {
        ...
        protected IEnumerable<Diagnostic> TemplateIdOfStandardValuesShouldMatchParentId(ICheckerContext context)
        {
            return from item in context.Project.Items
                ...
                select Error(Msg.C1000, "The Template ID of a Standard Values item should be match the ID of the parent item. To fix, moved the Standard Values item under the correct template", TraceHelper.GetTextNode(item), item.ItemName);
        }
