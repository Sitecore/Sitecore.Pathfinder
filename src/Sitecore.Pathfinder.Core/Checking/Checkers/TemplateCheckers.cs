...
namespace Sitecore.Pathfinder.Checking.Checkers
{
    ...
    public class TemplateCheckers : Checker
    {
        ...
        public IEnumerable<IDiagnostic> AvoidDeprecatedFieldType([NotNull] ICheckerContext context)
        {
            foreach (var template in context.Project.Templates)
            {
                foreach (var field in template.Fields)
                {
                    ...
                    {
                        yield return Warning(context, Msg.C1022, "Avoid deprecated field type", TraceHelper.GetTextNode(field.TypeProperty, field, template), $"The field type \"{type}\" is deprecated. Use the \"{newType}\" field type instead");
                    }
                }
            }
        }
        ...
        [ItemNotNull, NotNull, Check]
        public IEnumerable<IDiagnostic> AvoidDuplicateFieldNames([NotNull] ICheckerContext context)
        {
            foreach (var template in context.Project.Templates)
            {
        ...
        [ItemNotNull, NotNull, Check]
        public IEnumerable<IDiagnostic> AvoidEmptyTemplateSection([NotNull] ICheckerContext context)
        {
            return from template in context.Project.Templates
                from section in template.Sections
                where !section.Fields.Any()
                select Warning(context, Msg.C1118, "Avoid empty template section", TraceHelper.GetTextNode(section, template), section.SectionName);
        }
        ...
        [ItemNotNull, NotNull, Check]
        public IEnumerable<IDiagnostic> FieldIdTemplateFieldId([NotNull] ICheckerContext context)
        {
            return from item in context.Project.Items
                ...
                select Warning(context, Msg.C1024, "Field ID and Template Field ID differ", TraceHelper.GetTextNode(field.FieldIdProperty, field, item), $"FieldId: {field.FieldId.Format()}, TemplateFieldId: {templateField.Uri.Guid.Format()}");
        }

        [ItemNotNull, NotNull, Check]
        public IEnumerable<IDiagnostic> TemplateShouldHaveIcon([NotNull] ICheckerContext context)
        {
            return from template in context.Project.Templates
                where string.IsNullOrEmpty(template.Icon)
                select Warning(context, Msg.C1020, "Template should should have an icon", TraceHelper.GetTextNode(template), template.ItemName);
        }
        ...
        [ItemNotNull, NotNull, Check]
        protected IEnumerable<IDiagnostic> DefaultValueFieldIsObsolete([NotNull] ICheckerContext context)
        {
            return from item in context.Project.Items
                ...
                select Warning(context, Msg.C1121, "In a template field, the 'Default value' field is no longer used. To fix, clear the 'Default value' field and set the value on the Standard Values item", TraceHelper.GetTextNode(defaultValueField, item), item.ItemName);
        }
        ...
        public IEnumerable<Diagnostic> UseIdInsteadOfPath(ICheckerContext context)
        {
            return from template in context.Project.Templates
                ...
                select Warning(context, Msg.C1026, "Use IDs instead of paths in template fields", TraceHelper.GetTextNode(field.SourceProperty, field), $"The template field Source field contains the path \"{field.Source}\". It is recommended to use IDs instead.");
        }
        */
    }
}
