...
namespace Sitecore.Pathfinder.Compiling.FieldCompilers
{
    public class LayoutCompiler
    {
        ...
        protected virtual void WriteBool([NotNull] LayoutCompileContext context, [NotNull] XmlTextWriter output, [NotNull] ITextNode renderingTextNode, [NotNull] string id, [NotNull] string attributeName, [NotNull] string name, bool ignoreValue = false)
        {
            ...
            {
                context.CompileContext.Trace.TraceError(id + Texts.__Boolean_parameter_must_have_value__True__or__False_, renderingTextNode, attributeName);
                value = "False";
            }
        ...
        protected virtual void WriteDataSource([NotNull] LayoutCompileContext context, [NotNull] XmlTextWriter output, [NotNull] ITextNode renderingTextNode)
        {
            ...
            if (item == null)
            {
                context.CompileContext.Trace.TraceError(Texts.Datasource_not_found, dataSource);
                return;
            }
        ...
        protected virtual void WriteDevice([NotNull] LayoutCompileContext context, [NotNull] XmlTextWriter output, [NotNull][ItemNotNull] IEnumerable<Item> renderingItems, [NotNull] ITextNode deviceTextNode)
        {
            ...
            {
                context.CompileContext.Trace.TraceError(Texts.Device_element_is_missing__Name__attribute_, deviceTextNode);
            }
            else
            {
            ...
            if (layoutPath != null && !string.IsNullOrEmpty(layoutPath.Value))
            {
                ...
                if (l == null)
                {
                    context.CompileContext.Trace.TraceError(Texts.Layout_not_found_, layoutPath, layoutPath.Value);
                    return;
                }
        ...
        protected virtual void WritePlaceholder([NotNull] LayoutCompileContext context, [NotNull] XmlTextWriter output, [NotNull] ITextNode renderingTextNode, [NotNull] string id, [NotNull] string placeholders)
        {
            ...
            if (placeholderTextNode != null && !string.IsNullOrEmpty(placeholders))
            {
                if (placeholders.IndexOf("," + placeholder + ",", StringComparison.InvariantCultureIgnoreCase) < 0)
                {
                    ...
                    context.CompileContext.Trace.TraceWarning(string.Format(Texts._2___Placeholder___0___is_not_defined_in_the_parent_rendering__Parent_rendering_has_these_placeholders___1__, placeholder, text, id), placeholderTextNode);
                }
            }
        ...
        protected virtual void WriteRendering([NotNull] LayoutCompileContext context, [NotNull] XmlTextWriter output, [NotNull][ItemNotNull] IEnumerable<Item> renderingItems, [NotNull] ITextNode renderingTextNode, [NotNull] string placeholders)
        {
            ...
            {
                context.CompileContext.Trace.TraceError($"Unknown element \"{id}\".", renderingTextNode);
                return;
            }
            ...
            else
            {
                ...
                if (matches.Length == 0)
                {
                    context.CompileContext.Trace.TraceError($"Rendering \"{renderingItemId}\" not found.", renderingTextNode);
                    return;
                }

                if (matches.Length > 1)
                {
                    context.CompileContext.Trace.TraceError($"Ambiguous rendering match. {matches.Length} renderings match \"{renderingItemId}\".", renderingTextNode);
                    return;
                }
