...
{
    public class PageHtmlItemParser : PipelineProcessorBase<ItemParserPipeline>
    {
        public PageHtmlItemParser() : base(1000)
        {
        }

        protected override void Process(ItemParserPipeline pipeline)
        {
            var htmlFileTextNode = pipeline.TextNode.GetAttribute(PageHtmlPropertyCompiler.LayoutPage);
            ...
            var htmlFileProperty = item.GetProperty<string>(PageHtmlPropertyCompiler.LayoutPage) ?? item.NewProperty(PageHtmlPropertyCompiler.LayoutPage, string.Empty);
