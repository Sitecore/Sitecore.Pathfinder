namespace Sitecore.Pathfinder.Building.Builders.ItemFiles.ItemFileBuilders.LayoutItemFileBuilders
{
  using System;
  ...
  public class LayoutItemFileBuilder : ItemFileBuilderBase
  {
    ...
    public LayoutItemFileBuilder() : base(Layout)
    {
    }
    ...
    public override void Build(IItemFileBuildContext context)
    {
      ...
      var item = database.GetItem(context.ItemPath);
      if (item == null)
      {
        throw new RetryableBuildException(Texts.Text2003, context.FileName, 0, 0, context.ItemPath);
      }
      ...
      {
        throw new BuildException(Texts.Text2014, context.FileName);
      }
      ...
      if (!errors.Any())
      {
        var layoutItem = this.GetDestinationItem(context, item);
        this.SaveLayoutField(layoutItem, writer.ToString());
      }

      foreach (var error in errors)
      {
        context.BuildContext.Trace.TraceError(Texts.Text2026, error.Text, context.FileName, error.Line, error.Column);
      }

      foreach (var warning in warnings)
      {
        context.BuildContext.Trace.TraceWarning(Texts.Text2027, warning.Text, context.FileName, warning.Line, warning.Column);
      }

      if (errors.Any())
      {
        throw new BuildException(Texts.Text2020, context.FileName);
      }
    }
    ...
    [NotNull]
    protected Item GetDestinationItem([NotNull] IItemFileBuildContext context, [NotNull] Item item)
    {
      ...
      if (item == null)
      {
        throw new BuildException(Texts.Text2024, context.FileName);
      }
    ...
    private void WriteDevice([NotNull] IItemFileBuildContext context, [NotNull] XmlTextWriter output, [NotNull] IEnumerable<Item> renderingItems, [NotNull] Database database, [NotNull] XElement deviceElement, [NotNull] List<Message> errors, [NotNull] List<Message> warnings)
    {
      ...
      if (!string.IsNullOrEmpty(layoutPath))
      {
        ...
        if (layout == null)
        {
          throw new RetryableBuildException(Texts.Text2029, context.FileName, deviceElement, deviceElement.Attribute("Layout"), layoutPath);
        }
    ...
    private void WriteLayout([NotNull] IItemFileBuildContext context, [NotNull] XmlTextWriter output, [NotNull] Database database, [NotNull] XElement layoutElement, [NotNull] List<Message> errors, [NotNull] List<Message> warnings)
    {
