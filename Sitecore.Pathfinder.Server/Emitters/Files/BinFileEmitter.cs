namespace Sitecore.Pathfinder.Emitters.Files
{
  ...
  public class BinFileEmitter : EmitterBase
  {
    ...
    public override void Emit(IEmitContext context, ProjectItem projectItem)
    {
      ...
      var destinationFileName = "/" + PathHelper.NormalizeItemPath(PathHelper.UnmapPath(context.Project.ProjectDirectory, binFile.TreeNode.Document.SourceFile.SourceFileName));
      ...
      try
      {
        ...
        context.FileSystem.Copy(binFile.TreeNode.Document.SourceFile.SourceFileName, destinationFileName);
      }
      catch (Exception ex)
      {
        Log.Error($"Failed to copy assembly: {binFile.TreeNode.Document.SourceFile.SourceFileName} -> {destinationFileName}", ex);
      }
    }
  }
}
